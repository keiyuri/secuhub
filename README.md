...
**배포 전 점검 체크리스트**(2026-09-03 코드 리뷰 지적 — 위 전제가 실제로 지켜지는지 확인하는
절차가 문서화돼 있지 않았다. 애플리케이션이 자동으로 검증할 수 있는 항목이 아니라 배포 담당자가
수동으로 확인해야 한다):
- [ ] 게이트 TCP 포트(`securance.server.*` 설정의 리스닝 포트)가 게이트 세그먼트 외부(사무망/인터넷)
      에서 접근 불가능한지 방화벽 규칙으로 확인했다.
- [ ] 게이트 세그먼트와 백엔드/사무망 사이가 VLAN 또는 물리적으로 분리되어 있어, 사무망 단말이
      게이트 IP를 스푸핑할 수 없는지 확인했다.
- [ ] `tb_gate_dtl`에 등록된 IP 목록이 실제 배치된 게이트 장비와 정확히 일치하는지(더 이상 쓰지
      않는 IP가 남아있지 않은지) 확인했다.
