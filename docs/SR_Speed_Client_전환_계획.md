...
12. **Phase 10 — #18 대시보드 게이트 트리뷰**: 레거시 대시보드의 핵심 UI였던 LOC/GRP/DTL 3계층
    트리 + 연결상태 아이콘 + 우클릭 제어 메뉴. **최초 구현 완료(2026-08-12)** — 레거시 UI 원본을
    이 워크스페이스에서 확보하지 못해 이 요구사항만으로 신규 설계(사용자 확인). 대시보드에
    `/api/gate-tree` 폴링 기반 트리 위젯 추가, 우클릭 메뉴는 기존 리셋/모드변경/모터설정
    엔드포인트를 그대로 재사용. 상세는 [작업일지.md](작업일지.md) 0006 참고. **실 서버/브라우저
    검증 남음**(사내망 접근 불가로 컴파일/유닛테스트까지만 확인).
...
5. ~~`DevAutoLoginFilter` 제거~~ **완료(2026-08-12)** — 파일 및 배선/설정 키 제거(서버 계획서 5절 B9 참고).
