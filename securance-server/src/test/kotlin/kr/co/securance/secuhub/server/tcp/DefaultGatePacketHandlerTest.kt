...
class DefaultGatePacketHandlerTest {
    ...
    fun `상태 변경이 없어도 ACK는 반드시 회신한다`() = runBlocking {
        ...
        val handler = DefaultGatePacketHandler(registry, persister, RecordingDispatcher())
        val state = newState()
    ...
    fun `0x4D 상태 패킷은 레인 집합을 authoritative하게 교체하고 온라인으로 기록한다`() = runBlocking {
        val registry = RecordingRegistry()
        val handler = DefaultGatePacketHandler(registry, CountingPersister(), RecordingDispatcher())
        val state = newState()
    ...
    fun `제어 명령이 아닌 ACK는 제어 명령용 레인 FIFO를 소비하지 않는다`() = runBlocking {
        ...
        val handlerWithDispatcher = DefaultGatePacketHandler(registry, persister, dispatcher)
