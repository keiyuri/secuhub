...
/**
 * `/gates/locations/{id}/map` 실제 Thymeleaf 렌더링 검증(게이트 관리 팝업에서 "지도" 클릭 시
 * 지도 이미지가 표시되지 않던 문제 재현·고정, 2026-09-02).
 *
 * `location.locMapWidth`/`locMapHeight`(둘 다 nullable `Int?`)가 null인 레거시 데이터에서
 * 이미 좌표가 저장된 그룹 마커를 렌더링하면 SpEL이 null 산술(`grp.grpX * 100.0 /
 * location.locMapWidth`)로 500을 던져 지도 이미지를 포함한 화면 전체가 표시되지 않았다 —
 * 실제 MockMvc로 템플릿을 렌더링해 이 경로를 재현·고정한다.
 *
 * 500은 막되, 크기 정보가 없는 상태로 드래그를 계속 허용하면 저장 시 좌표가 (0,0)으로 조용히
 * 덮어써지는 후속 결함도 함께 막는다 — 크기 정보가 없을 때는 경고 배너를 노출하고 드래그 저장을
 * 비활성화(JS mapSizeValid 가드)했다. 드래그 활성화 여부는 클라이언트 JS라 MockMvc로 직접
 * 검증할 수 없으므로, 서버가 렌더링하는 경고 배너의 노출/비노출로 그 전제 조건을 고정한다.
 */
